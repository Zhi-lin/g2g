        ...
        for tokens in lines:
            ...
            for type in typelist:
                try:
                    ...
                    temp_type = self.__type_alphabet.get_index(ROOT_TYPE)
...
"""
第8列是source信息，第9列才是target信息（下标从0开始）
"""
class CoNLLUReaderMMoE(object):
    def __init__(self, file_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet_source,type_alphabet_target, pre_alphabet=None, pos_idx=4):
        self.__source_file = open(file_path, 'r')
        self.__word_alphabet = word_alphabet
        self.__char_alphabet = char_alphabet
        self.__pos_alphabet = pos_alphabet
        self.__type_alphabet_source = type_alphabet_source
        self.__type_alphabet_target = type_alphabet_target
        self.__pre_alphabet = pre_alphabet
        self.pos_idx = pos_idx
    ...
    def getNext(self, normalize_digits=True, symbolic_root=False, symbolic_end=False):
        ...
        for tokens in lines:
            ...
            for type in typelist:
                try:
                    temp_type = self.__type_alphabet_target.get_index(type)
                    temp.append(temp_type)
                except:
                    temp_type = self.__type_alphabet_target.get_index(PAD_TYPE)  # Jeffrey type不存在的情况
                    # temp_type = self.__type_alphabet.next_index
                    # self.__type_alphabet.next_index +=1
                    print("【ERROR arc_type:%s】" % type)
                    temp.append(temp_type)
            ...
            for type in src_typelist:
                ...
                #     src_temp.append(temp_type)
                try:
                    temp_type = self.__type_alphabet_source.get_index(type)
                    src_temp.append(temp_type)
                except:
                    temp_type = self.__type_alphabet_source.get_index(PAD_TYPE)
                    print("【ERROR arc_type:%s】" % type)
                    src_temp.append(temp_type)
            src_type_ids.append(src_temp)
